...
$lapClaim = "SELECT no_urut, no_claim, dealer, daerah, tglNota, toko, area_toko, b.brg, k.kat, pattern, dot, jenis_claim, kerusakan, tread, keputusan, nominal_pusat, nominal, noCM, tglCM
FROM(
SELECT no_urut, no_claim, dealer, daerah, tglNota, toko, area_toko, brg, pattern, dot, jenis_claim, kerusakan, tread, keputusan, nominal_pusat, nominal, noCM, tglCM
FROM tblNota
...
echo '
		<div class="headLap">
			<h3 id="marginLap">PT. Kharisma Utama Sentosa</h3>
	...
	foreach ($array as $index => $val) {
        if ($index==0) 
        {
         	echo '
	         		<tr>
	         			<tr>
	         				<td colspan="15" id="kategori">'.$val['kat'].'</td>
	         			</tr>';
	    } 
			echo '
	        			...
	        			if (preg_match("/[a-z,-]/i", $val['nominal'])) {
	        				...
	        				echo '<td class="kanan">'.$val['nominal'].'</td>';
	        				
	        			}else{
	        				echo '<td class="kanan">'.format_rupiah($val['nominal']).'</td>';

	        			}

						if (preg_match("/[a-z,-]/i", $val['nominal_pusat'])) {
							
							echo '<td class="kanan">'.$val['nominal_pusat'].'</td>';
	        				
	        			}else{
							echo '<td class="kanan">'.format_rupiah($val['nominal_pusat']).'</td>';
