...
$(document).ready(function() {
	...
	$('.datepicker').datepicker();
	 ...
	 $('#areaToko').typeahead({
		source: function(areaToko, result)
		{
		 $.ajax({
		  url:"fetchAutoCompleteAreaToko.php",
		  method:"POST",
		  data:{areaToko:areaToko},
		  dataType:"json",
		  success:function(data)
		  {
		   result($.map(data, function(item){
	 ...
	 $('#jenis_claim').typeahead({
		source: function(jenis_claim, result)
		{
		 $.ajax({
		  url:"fetchAutoCompleteJenisClaim.php",
		  method:"POST",
		  data:{jenis_claim:jenis_claim},
		  dataType:"json",
		  success:function(data)
		  {
		   result($.map(data, function(item){
	...
	$("#submitClaim").unbind('submit').bind('submit', function() {
		...
		var toko      = $("#toko").val();
		var areaToko  = $("#areaToko").val();
		var sales     = $("#sales").val();
		var brg       = $("#brg").val();
		var pattern   = $("#pattern").val();
		var dot       = $("#dot").val();
		var jenis_claim     = $("#jenis_claim").val();
		// var tread     = $("#tread").val();
		...
		if (no_urut == "") {
			$("#no_urut").after('<span class="error help-inline">No Urut Masih Kosong</span>');
			$("#no_urut").closest('#no_urut').addClass('error');
		}else{
			$("#no_urut").closest('#no_urut').addClass('success');
			$("#no_urut").find('.help-inline').remove();
		}

		// if (no_reg == "") {
		// 	$("#no_reg").after('<span class="error help-inline">No Register Masih Kosong</span>');
		// 	$("#no_reg").closest('#no_reg').addClass('error');
		// }else{
		// 	$("#no_reg").closest('#no_reg').addClass('success');
		// 	$('.help-inline').remove();
		// }
		...
		if (areaToko == "") {
			$("#areaToko").after('<span class="help-inline error">Nama Toko Masih Kosong</span>');
			$("#areaToko").closest('#areaToko').addClass('error');
		}else{
			$("#areaToko").closest('#areaToko').addClass('success');
			$(".help-inline").remove();
		}
		...
		if (jenis_claim == "") {
			$("#jenis_claim").after('<span class="error help-inline">Jenis Claim Produksi Masih Kosong</span>');
			$("#jenis_claim").closest('#jenis_claim').addClass('error');
		}else{
			$("#jenis_claim").closest('#jenis_claim').addClass('success');
			$('.help-inline').remove();
		}
...
/*		if (tread == "") {
		...
		if ( tgl && no_urut && no_claim && daerah && dealer && toko && areaToko && sales && brg && pattern && dot && jenis_claim ) {
			//ambil data form
			var form = $( this );
			//button loading
